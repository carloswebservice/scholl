    ...
    <body class="hold-transition skin-blue sidebar-mini">
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="box">
                                ...
                                <div class="box-body">
                                    <?php 
                                      foreach ($cursos["curso"] as $value) { ?>
                                        <center> <h3> Curso: <?php echo $value["cur_desc"] ?> </h3> </center>
                                          <table class="table table-bordered table-striped">
                                              ...
                                              <tbody>
                                                <?php 
                                                  foreach ($value["alumnos"] as $val) { ?>
                                                    <tr>
                                                      <td><?php echo $val["alum_id"]?></td>
                                                      <td><?php echo $val["alum_nombres"].' '.$val["alum_apellidos"]?></td>
                                                      <?php 
                                                        foreach ($val["asistencia"] as $v) { 
                                                          if ($v["asis_estado"]==0) { ?>
                                                            <td> <small class="label bg-red"><?php echo $v["cla_fecha"] ?> N</small></td>
                                                          <?php }else{ ?>
                                                            <td> <small class="label bg-green"><?php echo $v["cla_fecha"] ?> S</small></td>
                                                          <?php }
                                                          ?>
                                                        <?php }
                                                      ?>
              ...
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="box box-info">
                        ...
                        <form class="form-horizontal" id="form_docentes">
                          <div class="box-body">
        ...
        <script>
          ...
          function modificar(id){
            $.ajax({
                data:  'id='+id,
                url:   url+'Docentes/modificar',
                type:  'post',
                success: function(data) {
                  var datos = eval(data);
                  $("#id").val(datos[0]["doc_id"]);
                  $("#nombres").val(datos[0]["doc_nombres"]);
                  $("#apellidos").val(datos[0]["doc_apellidos"]);
                  $("#usuario").val(datos[0]["doc_usuario"]);
                  $("#clave").val(datos[0]["doc_clave"]);
                  $("#nuevo").modal("show");
                }
            });
          }
