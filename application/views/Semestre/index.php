    ...
    <body class="hold-transition skin-blue sidebar-mini">
                                        ...
                                        <tbody>
                                            <?php 
                                              foreach ($semestres as $value) { ?>
                                                <tr>
                                                  <td><?php echo $value->sem_id ?></td>
                                                  <td><?php echo $value->sem_desc ?></td>
                                                  <td> <small class="label bg-green"> Activo</small></td>
                                                  <td>
                                                    <button type="button" class="btn btn-warning btn-xs" onclick="modificar('<?php echo $value->sem_id ?>')">Modificar</button>
                                                    <button type="button" class="btn btn-danger btn-xs" onclick="eliminar('<?php echo $value->sem_id ?>')">Eliminar</button>
                                                  </td>
                                                </tr>
                                              <?php }
                                            ?>
                ...
                <div class="modal-content">
                  <div class="box box-info">
                        <div class="box-header with-border">
                          <h3 class="box-title">Registro Semestre</h3>
                        </div>
                        <form class="form-horizontal" id="form_semestre">
                          <div class="box-body">
        ...
        <script>
          ...
          function guardar(){
            ...
            $.ajax({
                data:  $("#form_semestre").serialize(),
                url:   url+'semestre/grabar',
                type:  'post',
          ...
          function modificar(id){
            $.ajax({
                data:  'id='+id,
                url:   url+'semestre/modificar',
                type:  'post',
                success: function(data) {
                  var datos = eval(data);
                  $("#id").val(datos[0]["sem_id"]);
                  $("#semestre").val(datos[0]["sem_desc"]);
                  $("#nuevo").modal("show");
                }
            });
          }
